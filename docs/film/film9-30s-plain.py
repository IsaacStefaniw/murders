...
phone that is not the app.

  MODE=landscape SHOTS=/path OUT=film9.html  python3 film9-30s-plain.py   # 1920x1080
  MODE=vertical  SHOTS=/path OUT=film9v.html python3 film9-30s-plain.py   # 1080x1920
"""
...
NAMES = ['today-before', 'int-2', 'int-3', 'int-4', 'int-5', 'coaches-tall',
         'workout', 'workout-autoreg', 'lib-card', 'lib-card-added']
...
.open .l{{font-family:var(--display);font-size:{74 if L else 84}px;line-height:1.08;letter-spacing:-.02em;color:var(--ink);opacity:0;transform:translateY(14px);transition:all .45s cubic-bezier(.2,.8,.2,1)}}
...
<div class="stage">
 ...
 <div class="open" id="open">
  <div class="l" id="o1">A gym app.</div>
  <div class="l" id="o2">A sleep app.</div>
  <div class="l" id="o3">A budget spreadsheet.</div>
  <div class="l" id="o4">A family calendar.</div>
  <div class="l k" id="o5">None of them know about each other.</div>
 </div>
 ...
 <div class="end" id="end"><div>
   ...
   <div class="tag">One app for the whole week.</div>
...
(async()=>{
 ...
 // 0–4.6 the problem, in plain words, no phone
 await wait(250); on($('o1')); await wait(650); on($('o2')); await wait(650); on($('o3')); await wait(650); on($('o4')); await wait(800); on($('o5'));
 await wait(1700); $('open').classList.add('out'); await wait(420);

 // 4.6–8.0 what it is
 show('today-before'); on($('phone'));
 await copy('IntentNorth is one iPhone app that plans <em>all of it</em> into your actual week.');
 await wait(3000);

 // 8.0–11.8 tell it once
 show('int-2'); await copy('Tell it about your life once.');
 await wait(700); await tapAt(22.8,52.7); show('int-3');
 await wait(600); await tapAt(13.1,51.5); await wait(150); await tapAt(63.2,51.5); show('int-4');
 await wait(600); await tapAt(58,63.7); show('int-5'); await wait(900);

 // 11.8–15.8 the seven areas
 show('coaches-tall',{y:-430,pan:-600,dur:2600});
 await copy('It builds a program for each part: training, food, sleep, habits, money, work, family.');
 await wait(3500);

 // 15.8–19.8 the day, decided, with the reason
 show('today-before'); await copy('Every morning, the day is already decided. <em>Every line says why.</em>');
 await wait(500); focus(10.5,9.8); await wait(3000); off($('focus'));
 ...
 // 24–28 the library
 show('lib-card'); await copy('323 practices, each with its evidence grade or the reason it has none. <em>One tap</em> and it’s in your week.');
...
out = pathlib.Path(os.environ.get('OUT', 'film9.html' if L else 'film9v.html'))
out.write_text(html)
