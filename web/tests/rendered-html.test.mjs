...
test("reps in reserve never returns, from anywhere", async () => {
  ...
  assert.match(html, /202 of the 318/, "the admission that makes the rating credible");
  // The costly signal is the whole reason the first pillar works.
  assert.match(html, /202 of them|202 of the 318/, "the honesty that makes the ratings credible");
